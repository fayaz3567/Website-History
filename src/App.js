...
class App extends Component {
  ...
  FillFunction = value => {
    ...
    if (newHistoryList.length == 0) {
      this.setStste({latestHistoryList: newHistoryList, isTrue: true})
    } else {
      this.setStste({latestHistoryList: newHistoryList})
    }
  }

  ChangeFunction = e => {
    this.setStste({searchInput: e.target.value})
  }
  render() {
          ...
          {!istrue && (
            <ul className="inner-holder">
              {newHistoryList.map(eachObject => (
                ...
                >
                  <p className="time">{eachOnject.timeAccessed}</p>
